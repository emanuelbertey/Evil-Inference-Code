...
impl MinGruConfig {
    pub fn init<B: Backend>(&self, device: &B::Device) -> MinGru<B> {
        ...
        let std_dev = (1.0 / self.input_features as f64).sqrt();
        ...
        MinGru { 
            linear_z: l_z.load_record(nn::LinearRecord {
                weight: Param::from_tensor(Tensor::random([self.input_features, hidden_size], Distribution::Normal(0.0, std_dev), device)),
                bias: Some(Param::from_tensor(Tensor::zeros([hidden_size], device))),
            }),
            linear_h: l_h,
...
fn parallel_scan_log<B: Backend>(log_coeffs: Tensor<B, 3>, log_values: Tensor<B, 3>) -> Tensor<B, 3> {
    ...
    let log_h0_plus_b_star = (x - x_max.clone()).exp().cumsum(1).log() + x_max;
