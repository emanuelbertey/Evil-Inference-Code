...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== xLSTMLarge EQUIVALENCE & CHAT GENERATION TEST ===");
    ...
    println!("¡Modelo inyectado con los pesos exactos de Python a Rust!");

    // PARTE 1: VERIFICAR EQUIVALENCIA LOGITS
    let b = x_shape[0];
    let s = x_shape[1];
    
    let x_tensor = Tensor::<MyBackend, 1, Int>::from_data(burn::tensor::TensorData::new(x_data.iter().map(|&v| v as i64).collect::<Vec<_>>(), [b * s]), &device)
        .reshape([b, s]);
    ...
    println!("Max Diff entre Logits (Python vs Rust): {:.10}", diff);
    ...
    // PARTE 2: GENERACIÓN DE TEXTO (Chat mode!)
    println!("\nGenerando texto en Rust usando 'xLSTMLarge::generate'...");
    let input_text = "The";
    let encoding = tokenizer.encode(input_text, false).unwrap();
    let tokens_ids: Vec<i64> = encoding.get_ids().iter().map(|&id| id as i64).collect();
