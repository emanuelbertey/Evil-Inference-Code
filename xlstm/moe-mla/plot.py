...
class PlotManager:
    ...
    def __init__(self, hf, save_dir=".", history_file=None,
                 plot_interval=256, json_prefix=""):
        self.hf = hf
        self.save_dir = Path(save_dir)
        if history_file is None:
            history_file = f"{json_prefix}train_history.json"
    ...
    def plot(self, step):
        ...
        path = self.save_dir / f"{self.json_prefix}plot_train_step_{step}.png"
    ...
    def plot_grad_moe(self, step):
        ...
        path = self.save_dir / f"{self.json_prefix}train_grad_moe_{step}.png"
        fig.savefig(path, dpi=100, bbox_inches="tight")
        plt.close(fig)
    ...
    def upload(self, step):
        ...
        for name in [f"{self.json_prefix}plot_train_step_{step}.png", f"{self.json_prefix}train_grad_moe_{step}.png"]:
